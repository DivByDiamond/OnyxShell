use super::{MAX_ARGS_PER, MAX_SEGMENTS, Pipeline, seg_to_args};
...
pub unsafe fn execute(line: &[u8], p: &Pipeline) {
    ...
    if p.n_segments == 1 && p.stdout_file.1 == 0 && p.stdin_file.1 == 0 {
        ...
        let cmd_args = seg_to_args(line, seg, &mut G_ARGS_BUF, &mut G_ARGS);
        if p.background {
    ...
    if p.n_segments == 1 {
        ...
        if is_inprocess_builtin(&line[a_off..a_off + a_len]) {
            ...
            let cmd_args = seg_to_args(line, seg, &mut G_ARGS_BUF2, &mut G_ARGS2);
            commands::dispatch(cmd_args);
            if stdout_fd > 2 {
                syscalls::close(stdout_fd as u64);
            }
    ...
    for seg_idx in 0..p.n_segments {
        ...
        if pid == 0 {
            ...
            let cmd_args = seg_to_args(line, seg, &mut G_ARGS_BUF3, &mut G_ARGS3);
            commands::dispatch(cmd_args);
            syscalls::exit(0);
        }
